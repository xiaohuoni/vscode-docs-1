...
使用 Visual Studio Code 启动和运行既快捷又简单。这是一个小下载，所以您可以在几分钟内安装，并给 VS Code 一个尝试。
...
VS Code 是轻量级的，应该在大多数可用的硬件和平台版本上运行。您可以查看系统要求以检查您的计算机配置是否受支持。
...
VS Code [每月](/updates)发布一个新版本，其中包含新特性和重要的错误修复。大多数平台都支持自动更新，当新版本可用时，系统会提示您安装新版本。您还可以通过运行 **Help** > **Check for Updates** 手动检查更新
...
>Note: 如果您希望按自己的计划更新 VS Code，可以[禁用自动更新](/docs/supporting/faq.md#how-do-i-opt-out-of-vs-code-autoupdates)。
...
如果您想尝试我们的夜间编译以尽早查看新功能或验证错误修复，您可以安装我们的 [Insiders build](/insiders)。工作人员编译每月安定的安装，您可以在同一台机器上自由工作。工作人员的编译与 VS Code 开发团队每天使用的编译是相同的，我们非常感谢人们尝试新功能并提供反馈。
...
首要的是，VS Code 是一个编辑器并且以占用小空间为傲。与传统的 IDE 不同，它往往包括除厨房水槽之外的所有内容，您可以根据您关心的开发技术调整您的安装。阅读完平台指南后，请务必阅读[附加组件](/docs/setup/additional-components.md)主题，以了解如何自定义 VS Code 安装。
...
扩展集成到 VS Code 的UI、命令和任务运行系统中，这样您就可以通过 VS Code 的共享界面轻松地使用不同的技术。查看 VS Code 扩展[市场](https://marketplace.visualstudio.com/vscode)以查看可用的内容。

## Next steps (下一步)

Once you have installed and set up VS Code, these topics will help you learn more about VS Code:

安装并设置 VS Code 后，这些主题将帮助您了解有关  VS Code 的更多信息：
...
如果您想让某个东西快速运行，请尝试[Node.js 教程](/docs/nodejs/nodejs-tutorial.md)演练，这将使您在几分钟内用 VS Code 调试 Node.js Web应用程序。
...
VS Code 是一个小的下载 (<100 MB)，磁盘占用空间小于 200 MB，因此您可以快速安装 VS Code 并进行尝试。
...
VS Code 不包括传统的**文件** > **新建项目**对话框或预安装的项目模板。根据您的发展兴趣，您需要添加[额外组件](/docs/setup/additional-components.md)和脚手架。有了像 [Yeoman](http://yeoman.io/) 这样的脚手架工具和通过 [NPM](https://www.npmjs.com/) 包管理器提供的大量模块，您一定会找到合适的模板和工具来创建您的项目。
